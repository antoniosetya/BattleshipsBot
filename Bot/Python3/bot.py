...
def fire_shot(opponent_map):
    ...
    for cell in opponent_map:
        if not cell['Damaged'] and not cell['Missed'] and (int(cell['X']) + int(cell['Y'])) % 2 == 0 and int(attackstatus[0]) == 0:
            ...
            if not cell['Damaged'] and not cell['Missed'] and int(cell['X']) == int(attackstatus[1])+1 and int(cell['Y']) == int(attackstatus[2]):
                valid_cell = cell['X'], cell['Y']
                targets.append(valid_cell)
            if not cell['Damaged'] and not cell['Missed'] and int(cell['X']) == int(attackstatus[1])-1 and int(cell['Y']) == int(attackstatus[2]):
                valid_cell = cell['X'], cell['Y']
                targets.append(valid_cell)
            if not cell['Damaged'] and not cell['Missed'] and int(cell['X']) == int(attackstatus[1]) and int(cell['Y']) == int(attackstatus[2])-1:
                valid_cell = cell['X'], cell['Y']
                targets.append(valid_cell)
            if not cell['Damaged'] and not cell['Missed'] and int(cell['X']) == int(attackstatus[1]) and int(cell['Y']) == int(attackstatus[2])+1:
                valid_cell = cell['X'], cell['Y']
            ...
            if int(attackstatus[0])== 2:
                if cell['Damaged']:
                    for cellss in opponent_map:
                        if int(cell['X'])+1 == int(cellss['X']) and int(cell['Y']) == int(cellss['Y']) and not cellss['Missed'] and not cellss['Damaged']:
    ...
    if not targets:
        ...
        fx = int(attackstatus[1])
        fy = int(attackstatus[2])
        direc = int(attackstatus[3])
    ...
    with open(os.path.join(output_path, attack_status), 'w') as f_out:
        ...
        f_out.write('\n')
        f_out.write('{}'.format(fx))
        f_out.write('\n')
        f_out.write('{}'.format(fy))
        f_out.write('\n')
        f_out.write('{}'.format(direc))
    return
